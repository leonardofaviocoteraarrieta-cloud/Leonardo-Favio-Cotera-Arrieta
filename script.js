...
// Puedes dejar actividades iniciales si quieres
...
function agregarActividad() {
  ...
  const archivo = document.getElementById("actividad-archivo").value; // ahora es un link de OneDrive

  if (!nombre || !archivo) {
    alert("Debes escribir un nombre y pegar el link público de OneDrive.");
    return;
  }

  const actividad = {
    nombre,
    archivo // ahora guardamos directamente el link
  };

  saveActividad(actividad);
  alert("Actividad agregada correctamente con link de OneDrive");
...
function mostrarActividades() {
  ...
  actividades.forEach((act) => {
    ...
    card.innerHTML = `
      <h4>${act.nombre}</h4>
      <a class="btn-ver" href="${act.archivo}" target="_blank">Ver</a>
      <a class="btn-descargar" href="${act.archivo}" download>Descargar</a>
    `;
